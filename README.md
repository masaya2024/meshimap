...
#### スコアが 100% でも、分母が縮んでいれば意味がない

`apps/api` の 100% は 2 回作り直しています。どちらも**スコアではなく分母を疑って**見つけました。

**1 回目：テストが 1 件も走らないまま Killed が積み上がっていた。**
`src/db/testing/local-d1.ts` がマイグレーションを 1 文ずつ `d1.exec()` に渡していました。
miniflare の D1 は workerd への loopback HTTP なので、`exec()` 1 回が TCP 接続 1 本です。
1 回のテスト実行で数千本が立ち、Stryker が並列に走らせると macOS の一時ポート
（49152-65535 の 16384 個、TIME_WAIT 約 30 秒）が尽きて
`connect EADDRNOTAVAIL 127.0.0.1:xxxxx` で `beforeAll` ごと落ちます。

`command` ランナーは**終了コードしか見ません**。落ちれば理由を問わず「その変異を殺した」と記録されます。
JSON レポートを開くと **129 変異すべての出力に `EADDRNOTAVAIL`** が入っていました。
出力を変えないはずの変異（近傍検索の枝刈りを消すもの）まで Killed になっていたのが決め手です。
全文を 1 回の `exec()` にまとめて解消しました。再発は
`local-d1.test.ts` の「D1 への往復回数」が**件数ではなく往復回数**で縛っています
（件数のテストでは 1 文ずつの実装に戻しても気づけないため）。

**2 回目：`// Stryker disable` で等価変異を無視したら、殺せる変異まで一緒に消えた。**
残った生き残り 8 件を「出力を変えられない等価変異」と判断して無視コメントを付けたところ、
`Killed 122 / Ignored 8` で 100% になりました。ここで分母が 129 から 122 に減っているのが不自然です。
中身を出すと、8 件のうち **6 件は殺せる変異**でした。

| 無視したもの                        | 実際は                                                        |
| ----------------------------------- | ------------------------------------------------------------- |
| 到達しないガード（`=== undefined`） | `→ true` と `→ !==` は「必ず投げる」に変わるので、殺せる      |
| 区切りの正規表現 `/\s+/u`           | `→ /\s/u` は等価だが、`→ /\S+/u` は区切りが逆転するので殺せる |

`Stryker disable` は**ミューテータ名でしか無効化できない**ため、
同じミューテータが作る「等価なもの」と「殺せるもの」を選り分けられません。
無視するのではなく、**無視しなくて済む形に書き直す**のが正解でした。

- 到達しないガードを `requireCondition()` という関数に括り出し、テストから直接呼べるようにした
- 語の分割を「区切りで割る」から「語そのものを拾う」（`match(/\S+/gu)`）に変え、
  空トークンという概念ごと無くした。捨てる処理が無くなったので、この正規表現の間違いは必ず出力に出る

結果が **132 変異 / Killed 132 / Ignored 0 / Survived 0** です。分母は 129 より増えています。
生き残りへの対応は 3 つしか認めないことにして、実装計画（Task 3-14 Step 9.5）に書き込みました。

| 生き残りの正体       | やること                           |
| -------------------- | ---------------------------------- |
| テストの穴           | テストを足す                       |
| 出力を変えない最適化 | 呼び出し回数で縛る                 |
| 到達不能             | 到達できる形に書き直してから、殺す |
...
| `apps/api`      | 20 ファイル / **414 件** | しきい値なし（`vitest.config.ts` に `thresholds` を置いていない）                        | **100%**（132 変異: Killed 132 / Timeout 0 / Survived 0 / Ignored 0） |
...
- 4 ワークスペースのテスト（[実測値](#4-実測値2026-09-15-時点)のコマンド）と
  ルートの `npm run test:mutation`（`@meshimap/api` / `@meshimap/core` / `@meshimap/geo` の 3 つを直列実行。約 8 分）
...
| `npm run test:mutation`                            | 全ワークスペースのミューテーションテストを実行 | 直列で約 8 分（api → core → geo） |

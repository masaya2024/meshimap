...
| 進行中           | Phase 3（Task 3-11 まで完了、3-12 を実行中）／ Phase 4 は計画書を執筆中                  |
| テスト件数       | `packages/core` 323 ＋ `packages/geo` 203 ＋ `apps/api` 244 ＋ `apps/mobile` 128 = 898   |
...
`apps/api` の 244 件は Task 3-11 完了時点の値。Task 3-12（シード生成）が作業中のため、
作業が終わるまで件数は増える。
